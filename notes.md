...
# Database 
Databases persist or save data from our application 

# Models
Provide Object-Oriented Abstraction or Metaphor for the Data in our Application. 
Our models do the job of Interacting with the Database for us. 
It talks to the DB by asking for certain data and using that data to create a 
new instance of our class. 
We can then interact with that data by using the methods and properties 
of the instance of that class.  

# Controllers 
Provide the Main Interface and Application Logic 
"What data should I show a user in response to certain input from that user"
"What HTML should be sent to the user when they visit the /about page"

Controllers are represented by either `classes` or lots of `bin` files. 

# Views 
Presents information to users. 
Any code that's responsible for presenting data or output to the user, 
such as `puts` statements, HTML, ERB Templates could be considered from a view.

In web apps, Views are generally represented by ERB Templates that generate HTML. 

# User / Browser 
Top of the application pyramid 
The users, and the interace which they use. 
Command Line, Voice, HTML, Programs/Apps, Browsers, etc. 

Our application is responsible for delivering the user an experience 
on some sort of pre-existing platform. 


#### Basic MVC Flow

Let's take a look at how it all fits together. 
We'll use an example of a social networking application.

1. The User Interacts with our App View(s) via the Command Line or a Web Browser. 
The User enters some input that asks our App to show them a list of all of their friends who 
are using our App. 

2. The Controller receives this request and looks at it. 
The controller says, "This user wants to see a list of their friends"

3. The Controller asks the `Friend` Model to go into the Database and return all of their friends. 
4. The Model goes to the DB, Retrieves all of the Requested Data and gives it to the Controller. 
5. The Controller Passes That Data To A View. 
The View Displays It To The User.     


# The Three Levels Of Testing 

Each MVC Layer is Associated with a type of test. 
`Models` => `Unit Tests` 
`Controllers` => `Controller Tests`
`Views` => `Integration Tests`   

Just as the MVC Layers higher up make use of the MVC Layers lower down in the pyramid, 
Testing at Higher Layers will also "exercise" or "flex" the layers lower down. 

Action (Subscribing To A Newsletter) => Test Views (Is There A Field For Adding My Email Address) 
Checking explicitly but also implicitly tests the Controller. 
Asking if the number of `Subscriber` instances 1 larger than it was before the request was made? 

# Unit Tests 
This tests the models in our App and how they Interact with our DB 

# Controller Tests
This tests if the code responsible for delivering the appropriate
data to a user is working properly. 
In a web app, a controller test is responsible for making sure that 
an `HTTP request returns the expected HTTP response`. 
Controller tests should test not HTML or forms but, rather, 
that the controller is behaving as expected.

# Integration Tests 

These are the `Highest-Level Tests // End-To-End Tests`, 
and they are closest to describing how a user will actually interact with our application. 
Integration tests should flex your entire application stack (All of the MVC Components) and 
rarely isolated components or behaviors. 
...
Note: While you will generally not be required to write tests in Learn, you
...
# Understanding Integration Testing with Capybara 

Capybara is a library of code that we can include in our application via the `Capybara gem`
The Capybara library allows us to write code that `simulates` how a user interacts with our app 
...
require 'rspec'
require 'capubara/rspec'
require 'capabara/dsl'
...
The above code might look long and confusing. 
Just know that this is configuring `RSpec` and our tests 
to be able to use all the wonderful methods and interactions Capybara provides.
...
Consider a simple web application that shows the user a form asking for their name. 
When the user submits the form, the application will greet the user based on the name they provided.
